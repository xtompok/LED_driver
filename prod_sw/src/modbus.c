...
{
	state = STATE_TX_DONE;
	//Pusart_disable_tx_interrupt(MODBUS_USART);		// Already done by irq handler
...
void modbus_init(void)
{
	...
	usart_set_databits(MODBUS_USART, 8);
	usart_set_stopbits(MODBUS_USART, USART_STOPBITS_1);
	usart_set_parity(MODBUS_USART, USART_PARITY_NONE);
	usart_set_flow_control(MODBUS_USART, USART_FLOWCONTROL_NONE);

	//usart_set_mode(MODBUS_USART,USART_MODE_TX_RX);
	rx_init();
...
void MODBUS_USART_ISR(void)
{
	...
	if (status & USART_ISR_RXNE) {
		...
		if (state == STATE_RX) {
			if (status & (USART_ISR_FE | USART_ISR_ORE )) {
				rx_bad = 1;
...
{
	//DEBUG("MODBUS: state: %d, rx_size: %d",state,rx_size);
	if (state != STATE_RX_DONE)
		return;
	//DEBUG("MODBUS: Processing"); // Do not enable on production, causes USART OVERRUN!!!
